...
// Accepts:  { tier: "basic" | "pro" | "agency", email?: string }
// Returns:  { url: string }  — redirect to Dodo hosted checkout
//
// Required env vars (set in Vercel dashboard):
//   DODO_PAYMENTS_API_KEY      — your Dodo Payments API key
//   DODO_PRODUCT_BASIC         — product ID for Basic plan  ($9/mo)
//   DODO_PRODUCT_PRO           — product ID for Pro plan    ($29/mo)
//   DODO_PRODUCT_AGENCY        — product ID for Agency plan ($99/mo)
...
const DODO_PRODUCT_AGENCY = process.env.DODO_PRODUCT_AGENCY;
...
export default async function handler(req, res) {
  ...
  if (!tier || !["basic", "pro", "agency"].includes(tier)) {
    return res
      .status(400)
      .json({ error: "Invalid tier. Use 'basic', 'pro', or 'agency'." });
  }
